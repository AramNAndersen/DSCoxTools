# Computation of drug sensitivity metrics for https://doi.org/10.1101/2022.10.11.509866
...
# Compute raw AUC
df_dose.responses$ID <- paste0(df_dose.responses$drug,"_Patient ", df_dose.responses$Patient.num,"_Plate ",  df_dose.responses$platenumber)
df_dose.responses$inhibition_percent <- 100 - df_dose.responses$response.raw*100
df.AUC.raw <- df_dose.responses %>%
  group_by(ID, Patient.num, drug, platenumber) %>%
...
df_scores <- cbind(df.AUC.raw, df.DSS_metrics, df.EC50_metrics, df.believe)

save(df_scores, file = paste0(path,"/Drug sensitivity metrics.RData"))
...
pdf(file = paste0(path, "/Results 20220711/Drug sensitivity metric correlations_NOR.pdf"),
    width=7.3, height=7)
pairs(mat[,],
...
tiff(file = paste0(path, "/Results 20220711/Drug sensitivity metric correlations_NOR.tiff"),
...
rm(df_dose.responses, mat,
   drug_wells_, xpr_tbl,
   df.AUC.raw, df.believe, df.DSS_metrics, df.EC50_metrics,
   list.believe, list.DSS_metrics, list.EC50)
#----
